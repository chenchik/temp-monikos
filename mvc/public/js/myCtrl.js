  ...
  $scope.getSchoolLists = function() {
    ...
    $http.post(url_getschools, data, config)
      .then(function(response) {
        ...
        if (response.data.records.length < 1) {
          $(".list-collection-block").append(
            '<p id="noListsMessage">This school does not have any lists.</p>'
          );
        } else {
          for (var i in response.data.records) {
  ...
  */
  ...
  $scope.launchGame = function() {
    if ($scope.passedId != undefined) {
      window.location = window.location.origin +
        "/mvc/public/games/menu/" + $scope.passedId;
    } else {
...
$(document).ready(function() {
  $('.list-collection-block').on('click', '.list_block .selectList',
    function() {
  ...
  $('.list-collection-block').on('click', '.list_block .deleteList',
    function() {
      $(this).parent().remove();
    });
  var selected;
