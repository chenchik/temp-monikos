...
app.controller('matchingCtrl', function($scope, $http) {
    ...
    $http.post(url, data, config)
      .then(
        function(response) {
          //success
          console.log(response);
          document.cookie = "user_id=" + response.data[0].user_id +
            "; expires=" + (Date.now() + (86400 * 30)) + "; path=/";
      ...
      .then(function(response) {
        ...
        for (var i = 0; i < $scope.names.length * 2; i++) {
          if (i < $scope.names.length) {
            ...
            objects[i] = {
              drug: $scope.names[i - $scope.names.length],
              front: $scope.names[i - $scope.names.length].Brand,
              clicked: 'N',
              correct: 'N',
              active: 'N'
            };
  ...
  $scope.clicked = function(front) {
    ...
    if ($scope.numClicked > 2) {
      ...
      for (var i = 0; i < shuffledObjects.length; i++) {
        if (shuffledObjects[i].clicked == "Y" && shuffledObjects[i].front ==
          front) {
    ...
    if ($scope.numClicked == 2) {
      ...
      if ($scope.firstCard.front == $scope.secondCard.drug.Brand ||
        $scope.firstCard.front == $scope.secondCard.drug.Generic) {
        $scope.correct = "Y";
        $scope.numCorrect += 2;
        $scope.right++;
  ...
  $scope.determineWinner = function(id) {
    ...
    $http.post(url, data, config)
      .then(function(response) {
        ...
        $scope.updatecapsules(id, user1, user2, user1score, user2score,
          bet);

      });
  }

  $scope.updatecapsules = function(id, usr1, usr2, usr1score, usr2score,
    thebet) {
    var data = $.param({
      challengeid: id,
      user1: usr1,
  ...
  $scope.deleteChallenge = function(id, usr1, usr2, usr1score, usr2score,
    thebet, score1, score2) {
    var data = $.param({
      challengeid: id
    });
    ...
    $http.post(url, data, config)
      .then(function(response) {
        ...
        if (score1 > score2) {
          $('.challengeCompleteText').html("YOU WON " + thebet +
            " CAPSULES! Looks like you're quicker than " + $scope.getUser1() +
            ". Click the button below to return to the game menu.");
        } else if (score1 < score2) {
          $('.challengeCompleteText').html("You lost " + thebet +
            " capsules against " + $scope.getUser1() +
            ". Click the button below to return to the game menu.");
        } else {
          $('.challengeCompleteText').html("You tied.\nNeither you or " +
            $scope.getUser1() +
            " are awarded any capsules for this challenge. Click the button below to return to the game menu."
          );
        }
        challengeComplete();
      });
  ...
  $scope.flashPills = function(caps) {
    ...
    $("#updated-capsules-indicator").css({
