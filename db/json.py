# "$--------#--------!-------*-------@-------|"
# this is the json generator for drug.
# 1, run, copy only ONE drug information into the console: input should start with "$"
# 2, make sure the drug info follow the order : Generic, Menemonic, Brand, Class, \
#                                               Indication, Renal, Hepatic, Side effects, Mechanism, \
#                                               Black, Resource.
# 3, Remove all the blank lines
# 4, when there is error, recheck whether inputs follows: step 2 and step 3
# 5, for the output, only copy the last line which is the correct json.
...
def parse_non_spe(string,current):
    first=current;
    while current < len(string) and (string[current]!="#") and  (string[current]!="$") and (string[current]!="@") and (string[current]!="|") and (string[current]!="*") and (string[current]!="!"):
        current = current + 1
